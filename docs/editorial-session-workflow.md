...
Status: implementation contract with functional background, hardened resume pass in `v0.3.5`, startup crash recovery hardening in `v0.3.6`, long-running agent diagnostics in `v0.3.7`, and DeepSeek API peer support in `v0.3.11`.

Maestro's core workflow starts from an operator prompt and an active editorial protocol. The app must not deliver a final text until Claude, Codex, Gemini, DeepSeek, and Maestro's deterministic local checks all return `READY` in the same accepted round.
...
- Claude, Codex, Gemini, and DeepSeek are called in background to review the draft.
...
- If the operator has imported a new protocol before resuming, that protocol is passed to Claude, Codex, Gemini, and DeepSeek; the previous `protocolo.md` is preserved as a local `protocolo-anterior-*.md` artifact before the new protocol becomes active.
...
- DeepSeek uses the direct API path and records `deepseek-api` artifacts in the same `agent-runs` directory as CLI agents.
...
7. A final text is accepted only when Claude, Codex, Gemini, DeepSeek, and MaestroPeer all return `READY` in the same round.
...
`texto-final.md` is created only after Claude, Codex, Gemini, DeepSeek, and MaestroPeer are unanimously `READY`.
