  ...
  if (typeof value === 'object') {
    return Object.fromEntries(
      Object.entries(value as Record<string, unknown>)
        .slice(0, 120)
        .map(([key, item]) => [key, shouldRedactKey(key) ? '<redacted>' : sanitize(item, depth + 1)]),
    );
  }
  return String(value);
}
...
export async function logEvent(event: LogEvent) {
  ...
  try {
    ...
    const logger = event.level === 'error' || event.level === 'fatal' ? originalConsole.error : originalConsole.debug;
...
export function installGlobalDiagnostics() {
  ...
  void logEvent({
    ...
    context: {
      ...
      capture: ['ui_events', 'frontend_errors', 'unhandled_rejections', 'console_warn_error', 'network_state'],
    },
  });
