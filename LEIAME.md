...
Quando o provedor oferece cache de prompt sem degradar qualidade, o Maestro usa isso apenas para reduzir custo: OpenAI/Codex e Grok/xAI enviam `prompt_cache_key`, Anthropic/Claude marca o bloco `system` estavel com cache efemero, e Gemini/DeepSeek usam cache implicito/automatico do provedor. Perplexity/Sonar nao documenta controle explicito de prompt cache comparavel para este fluxo, entao o Maestro nao inventa campo de cache no payload. Logs e artefatos gravam somente hash/metadados nao secretos e contadores de tokens de cache quando o provedor retorna esses dados.
